...
def let(args, scope):
    ...
    for n, v in zip(names, values):
        if isinstance(n, Atom):
            scope[n.value] = v
        elif isinstance(n, Property):
            o = scope[n.value[0]]
            for p in n.value[1:-1]:
                o = o[p]
            o[n.value[-1]] = v
    return Atom('none')
...
def operator_(args, _):
    ...
    def inner(innerargs, scope):
        ...
        res = None
        for e in body:
            res = eval_list(e, OperatorScope(argsdict, scope))
        return res
    return inner

def def_operator(args, scope):
    name = args[0].value[0].value
    listargs = List(args[0].value[1:])
    scope[name] = operator_([listargs] + args[1:], scope)
    return Atom('none')
